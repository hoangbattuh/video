...
from src.backend.video_processing.video_cutter import cut_by_segments, cut_by_duration, cut_selected_segment
from src.backend.video_processing.video_merger import merge_videos
from src.backend.video_processing.video_editor import remove_audio
...
class VideoEditorPro(tk.Tk):
    def __init__(self):
        ...
        self.progress_label = ttk.Label(progress_stop_frame, text="")
        ...
        # Thanh trạng thái
        self.status_bar = ttk.Label(self, text="Sẵn sàng", relief=tk.SUNKEN, anchor=tk.W)
        self.status_bar.pack(side=tk.BOTTOM, fill=tk.X)

    def update_progress_safe(self, value, message=""):
        self.after(0, self._update_progress, value, message)
    ...
    def cut_by_segments(self):
        ...
        threading.Thread(target=cut_by_segments, args=(
            input_path, output_dir, num_segments, self.stop_flag, self.update_progress_safe, self.set_status_safe, messagebox
        )).start()
    ...
    def cut_by_duration(self):
        ...
        threading.Thread(target=cut_by_duration, args=(
            input_path, output_dir, segment_duration, self.stop_flag, self.update_progress_safe, self.set_status_safe, messagebox
        )).start()
    ...
    def cut_selected_segment(self):
        ...
        threading.Thread(target=cut_selected_segment, args=(
            input_path, output_dir, start_time, end_time, self.stop_flag, self.update_progress_safe, self.set_status_safe, messagebox
        )).start()
    ...
    def start_remove_audio(self):
        if not self.audio_selected_video_path:
            messagebox.showwarning(NOTIFICATION_TITLE, "Vui lòng chọn video để xóa âm thanh.")
            return
